...
ChessDraw create_draw(ChessBitboard board, ChessPosition oldPos, ChessPosition newPos, ChessPieceType peasantPromotionType)
{
    ...
    int is_first_move = get_was_piece_moved(piece) == 0 ? 1 : 0;
    ChessDrawType draw_type = determine_draw_type(board, oldPos, newPos, peasantPromotionType);
    ChessColor drawing_side = get_piece_color(piece);
    ChessPieceType drawing_piece_type = get_piece_type(piece);
    ChessPieceType taken_piece_type = 
